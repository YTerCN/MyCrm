...
<%
	...
	for (int i = 0;i<dList.size();i++){
		if ("0".equals(pMap.get(dList.get(i).getValue()))){
			point = i;
			break;
		}
	}
...
<script type="text/javascript">
	...
	function tranHistoryList() {
		$.ajax({
			url:"workbench/transaction/getHistoty.do",
			data:{
				"tranId":"${t.id}"
			},
			type:"get",
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
            ...
            //如果当前阶段可能性为0，则前七个阶段的图标为  黑圈  ，后两个阶段一红叉一黑叉
            if ("0".equals(currentPossibility)){
                for (int i=0;i<dList.size();i++){
                    DicValue dv = dList.get(i);
                    String listStage = dv.getValue();
                    String listPossibility = pMap.get(listStage);
                    //如果遍历出来的阶段可能性为0，则为最后两个阶段，一红叉一黑叉
		...
		<%
                ...
                //当前阶段可能性不为0，则前七个阶段可能为绿圈、绿色标记、黑圈，  后两个阶段为黑叉
                //准备当前阶段的下标
                int index = 0;
                for (int i = 0;i<dList.size();i++){
                    DicValue dv = dList.get(i);
